SSposnegRichards <- structure(function(x, Asym = NA, 
    ...
    modno) {
    try(pnmodelparamsbounds <- get("pnmodelparamsbounds", envir = .GlobalEnv), 
        silent = TRUE)
    fractM <- M
    fractRM <- RM
    if (modno > 19) {
    ...
    pars <- as.numeric(getInitial(mass ~ SSposnegRichards(age, 
        Asym, K, Infl, M, RAsym, Rk, Ri, RM, modno = 22), data = posneg_data))
    ...
    subdata <- subset(posneg_data, as.numeric(row.names(posneg_data)) < 
        53)
    ...
    richardsR20.lis <- nls(mass ~ SSposnegRichards(age, Asym = Asym, 
        ...
        RM = 1, modno = 20), data = logist_data)
